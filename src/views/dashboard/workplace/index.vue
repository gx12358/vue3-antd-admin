...
<template>
  <g-pro-page-container :use-page-card="false">
    <template #contentRender>
      <div class="text-hidden-1 text-20px leading-32px font-600">{{ currentRoute.meta.title }}</div>
      <div class="flex items-center justify-between mt-12px flex-wrap">
        <div class="flex gap-24px lt-md:w-full">
          ...
          <div class="flex flex-col py-10px justify-between gap-10px">
            ...
            <div :style="colorTextDescription">前端工程师 | （REACT，VUE，UNIAPP、TARO）平台</div>
          </div>
        </div>
        <div class="flex items-center gap-32px lt-md:w-full lt-md:mt-20px" v-if="Object.keys(projectCount).length">
    ...
    <a-row :gutter="24" >
      <a-col :xl="16" :lg="24" :md="24" :sm="24" :xs="24">
        ...
        >
          ...
          <GProCard
            class="card-item"
            :colSpan="{ xl: 8, lg: 8, md: 12, sm: 24, xs: 24 }"
          ...
          >
            <div class="flex flex-col gap-8px">
              ...
              <div :style="colorTextDescription" class="text-hidden-2 leading-22px">{{ item.description }}</div>
        ...
        <GProCard title="动态" header-bordered :loading="mailNoticeLoading">
          <div class="flex py-16px gap-16px mail-item" :style="colorFillContent" v-for="item in mailNoticeList" :key="item.id">
      ...
      <a-col style="padding: 0 12px" :xl="8" :lg="24" :md="24" :sm="24" :xs="24">
        <GProCard class="mb-24px" title="快速开始 / 便捷导航" header-bordered>
          <div class="flex flex-wrap">
            ...
            <a-button ghost type="primary" size="small">添加</a-button>
          </div>
        </GProCard>
        ...
        <GProCard title="团队" header-bordered :loading="groupLoading">
          <a-row :gutter="10">
            <a-col :span="12" v-for="item in groupList" :key="item.id">
              <div class="py-12px">
