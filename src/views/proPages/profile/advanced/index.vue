  ...
  <g-pro-page-container :use-page-card="false" :loading="loading">
    <template #contentRender>
      <div class="flex items-center justify-between">
        ...
        <a-dropdown-button type="primary" placement="bottomRight" v-if="isMobile">
          <template #icon>
            <down-outlined />
          </template>
          <template #overlay>
      ...
      <div class="w-full flex gap-20px mt-12px lt-xs:flex-wrap lt-sm:flex-wrap">
        <a-descriptions size="small" :column="[ 'sm', 'xs' ].includes(colSize) ? 1 : 2">
          <a-descriptions-item label="创建人">{{ data.createName }}</a-descriptions-item>
          ...
          <a-descriptions-item label="生效日期">{{ data.effectTime }}</a-descriptions-item>
          ...
          <a-popover v-if="status === 'process'" placement="topLeft" arrowPointAtCenter>
            <template #content>
              <div class="w-160px">
                ...
                <div class="mt-4px"> 耗时：2小时25分钟</div>
              </div>
            </template>
