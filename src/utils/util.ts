...
import { isFunction, scrollTo, isNumber, isObject } from '@gx-design-vue/pro-utils'

export type NumberToShow = {
  numberValue: string;
  countStr: string;
  joinStr: string;
}
...
export function toConvertNumberShow(
  ...
  if (!isNumber(number) || options?.unit) return {
    numberValue: `${number}`,
    countStr: countStr,
      ...
      newNum =
        (i === 0 && strArr[i] === '0'
          ...
          : i > 0 && strArr[i] === '0' && strArr[i - 1] === '0'
            ? ''
            : (num < 20 && i > 0 ? '' : changeNum[strArr[i]]) + (strArr[i] === '0' ? unit[0] : unit[i])) + newNum
    }
    return newNum
  }
